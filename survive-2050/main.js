...
function costOf(choice, tier){
  ...
  let c = (s>=20 ? 180 : s>=10 ? 110 : 50);     // 친환경 자본집약일수록 비쌈
  c += (tier-1)*40;                              // 후반 위기일수록 메가프로젝트 단가↑
  c += Math.round(Math.max(0, -choice.fx.temp)*60); // 적극적 기온 감축일수록 비쌈
  return c;
}
...
function choose(choiceIdx){
  ...
  // 「예산」 난이도: 비용 차감(예산 부족 시엔 가장 싼 보기만 강제 허용)
  if(diffCfg().budget){
    const costs = scene.choices.map(ch=>costOf(ch, item.tier));
...
function renderCurrentScenario(){
  ...
  // 예산 모드: 보기별 비용 + 최저가(항상 구매 가능) 산출
  const costs = budgetMode ? scene.choices.map(c=>costOf(c, item.tier)) : null;
  ...
  const choicesHTML = scene.choices.map((c, idx)=>{
    ...
    // 우측 뱃지: 예산 모드=비용 / 기본(힌트 표시 시)=SDG / 그 외=없음
    let rightBadge = '';
    if(budgetMode){
      const cost = costs[idx];
      const afford = cost <= G.budget || idx === cheapest;
      rightBadge = `<span class="text-[10px] rounded-full px-1.5 py-0.5 font-bold shrink-0 ${afford?'bg-amber-400/15 text-amber-300 border border-amber-400/30':'bg-red-500/15 text-red-300 border border-red-500/30'}">💰 ${cost}억</span>`;
    } else if(!hide){
    ...
    const headLeft = (hide || budgetMode)
      ? `<div class="font-bold text-emerald-400 text-xs">[선택 ${L}]</div>`
    ...
    // 예산 부족으로 살 수 없는 보기는 비활성화(최저가는 항상 허용)
    ...
    const onclick = afford ? `onclick="choose(${idx})"` : 'disabled';
    return `
      <button ${onclick} class="${cls}">
        ${head}
        ${c.label}${afford?'':' <span class="text-[10px] text-red-300">· 예산 부족</span>'}
      </button>`;
  }).join('');
  ...
  if(budgetMode){
    ...
    budgetBar = `
      <div class="mb-3 rounded-xl bg-amber-500/10 border border-amber-500/25 p-2.5">
        <div class="flex justify-between text-[11px] font-bold text-amber-200 mb-1"><span>💰 남은 예산</span><span>${G.budget}억 / ${G.budgetTotal}억</span></div>
  ...
  ov.innerHTML = `
    <div class="w-full h-[100dvh] overflow-y-auto flex justify-center"
      ...
      <div id="shareCard" class="relative w-full max-w-[400px] my-auto flex flex-col rounded-[28px] overflow-hidden animate-pop"
        ...
        ${(d.budget && G) ? `<div class="px-5 pt-2 shrink-0"><div class="rounded-xl bg-amber-500/10 border border-amber-500/25 p-2.5 text-center text-[11px] font-bold text-amber-200">💰 남은 예산 ${G.budget}억 / ${G.budgetTotal}억</div></div>` : ''}
...
function startGame(){
  ...
  G = { stats: freshStats(), history: [], currentStep: 0, gameQueue: [], modifier: null, renderingHalted: false, rollTimers: [], budget: 0, budgetTotal: 0 };
  buildQueue();
